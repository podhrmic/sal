//! SAL Infinite-State Bounded Model Checker (Rust reimplementation, SMT-based).
...
use sal_core::SalEnv;
use sal_engine::smt::{smt_bmc_search, smt_k_induction, SmtResult as BmcResult};
...
fn main() -> ExitCode {
    ...
    let target = if let Some(a) = args.opt("assertion") {
        ...
        eprintln!("Usage: sal-inf-bmc [options] <context-name> <assertion-name>");
    ...
    let result = if induction {
        ...
        smt_k_induction(&flat, &prop, depth, &lemmas)
    } else {
        smt_bmc_search(&flat, &formula, depth, &lemmas, args.flag("iterative"))
    };
